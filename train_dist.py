...
def decode_dist(dist_view):
    dist = image_color2idx(dist_view, rgb=True).astype(np.float32).reshape([conf.height, conf.width, 1])
    return dist
...
def random_batch(batch_size):
    ...
    dist_color = tf.image.decode_png(tf.read_file(data_paths[0]), 0)
    dist = tf.py_func(decode_dist, [dist_color], [tf.float32])[0]

    seg = tf.image.decode_png(tf.read_file(data_paths[1]), 0)
    label = tf.py_func(decode_label, [seg], [tf.uint8])[0]

    dist.set_shape([conf.height, conf.width, 1])
    label.set_shape([conf.height, conf.width, conf.num_classes])

    dist_batch, label_batch = tf.train.batch([dist, label], batch_size, num_threads=8)
    return dist_batch, label_batch
...
def train(model_range, seg_range, swi_range, dis_range, rot_range, batch_size=1, num_epochs=10, learning_rate=1e-4, log_dir='', checkpoint_path='', retrain=False):
                    ...
                    for swi in swi_range:
                        for dis in dis_range:
                            for rot in rot_range:
                                ...
                                dist_view_path = conf.dist_view_path(model, mesh, view_name)
                                seg_view_path = conf.segmentation_view_path(model, mesh, seg, view_name)
                                file_paths = '{} {}\n'.format(dist_view_path, seg_view_path)
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        feat_opt = tf.train.AdamOptimizer(learning_rate)
        clas_opt = tf.train.AdamOptimizer(learning_rate)

        dist_batch, label_batch = random_batch(batch_size)

        #with tf.device('/gpu:0'):
        net = RHBC('train', dist_batch, label_batch, feat_opt, clas_opt)
